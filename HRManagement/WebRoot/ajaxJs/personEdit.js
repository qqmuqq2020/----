...
$(function(){
	...
	if(pid==0){
			$("#pidfor").append("<input type='text'name='nowaddress' value='' />");
	}else{
		$("#pidfor").append("<font margin:0 auto; name='nowaddress' />");
	}
...
//				$("[name='idcardno']").val(item.idCardNo);
				...
				$("[name='nowaddress']").text(item.nowAddress);
				$("#gid").val(item.gid);
				staffid=item.staffID;
				if(item.ready2!=""&&item.ready2!=null&&item.ready2!=undefined){
					var str = "	<div><label for='NowAddress' >发行员：</label><font margin:0 auto; name='nowaddress' >"+item.staffnameup+"</font></div>"
					$("#pidfor").append(str);
				}
			});
